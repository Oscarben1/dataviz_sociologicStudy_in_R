...
data2 <- dfPortugais %>% filter(Pstatus == 'A') %>% group_by(Mjob) %>% summarise_all(funs(mean))

data3 <- dfMaths %>% filter(Pstatus == 'A', guardian == "mother") %>%
  select(Medu) %>% count(Medu)

data4 <- dfPortugais %>% filter(Pstatus == 'A') %>% group_by(Medu) %>% summarise_all(funs(mean))
...
plot2 <- ggplot(data2, aes(x = Mjob, y = health)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Mjob)) +
...
plot4 <- ggplot(data4, aes(x = Medu, y = health)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Medu)) +
...
<li> Le travail des parents : On constate une meilleure santé lorsque lepère ou la mère travaille dans le secteur de la santé.</li>
<li> La nature de la famille : En prenant l'évènement contraire on voit que les familles monoparentales où la garde est à la mère rencontre des difficultés et que ca se passe relativement bien dans les autres cas. </li>
