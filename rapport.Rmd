...
Puis, un deuxième axe de reflexion portera sur **les relations familiales** des élèves, en analysant quels sont les variables qui expliquent le mieux **les bonnes relations familiales** d'un élève.<br>
Enfin, nous avons choisi de travailler sur **le plan santé**, que doit faire un élève pour avoir **la meilleure santé possible**. Notre analyse comporte 30 graphs, ce qui est assez long mais nous avons jugé chaque graph présent comme **indispensable à la bonne compréhension** des axes de développement, afin que l'analyse soit la plus **claire** et **précise** possible.
...
<ul>
  ...
  <li>studytime (temps de travail) : échelle allant de 1 à 4 - 1 : **0 - 2 h**, 2 : **2 - 5 h**, 3 : **5 - 10 h**, 4 : **> 10 heure**</li>
...
donner répartitions filles/garçons + campagne/ville... quelques données importantes + DIAGRAMME CORELATIONS ???
...
Voici le temps de travail en moyenne des garcons : **`r tempsMoyenGarcons`**. <br>

En comparaison au temps moyen des filles en Maths, qui est plus élevé : **`r tempsMoyenFilles`**.<br>
...
fJob <- ggplot(df, aes(x = Fjob, y = mean_Gmean)) + 
    ...
    labs(x = "Job du père", y = "Gmean", title = "Moyenne selon le travail du père") + 
    theme(legend.position = "none") +
...
ggplot(df, aes(x = higher, y = mean_Gmean)) +
    ...
    labs(title = "Notes en fonction de la volonté de faire des études supérieure", x = "Volonté de faire des études sup", y = "Moyenne des notes") +
  scale_fill_brewer(palette = "Pastel2") + 
    theme(legend.position = "none") +
...
ggplot(df, aes(x = freetime, y = mean_Gmean)) +
    ...
    geom_line(color = "blue") +
    labs(title = "Notes selon le temps libre", x = "Temps libre", y = "Moyenne des notes") +
    scale_fill_discrete(type = "viridis") +
    theme(legend.position = "none")
...
Walc <- ggplot(df, aes(x = Walc, y = mean_Gmean)) +
    ...
    geom_line(color = "blue") +
    labs(title = "Notes selon cons. alc. week-end", x = "Consommation d'alcool le week-end", y = "Moyenne des notes") +
    scale_fill_discrete(type = "viridis") +
    theme(legend.position = "none")
...
Dalc <- ggplot(df, aes(x = Dalc, y = mean_Gmean)) +
    ...
    geom_line(color = "blue") +
    labs(title = "Notes selon cons. alc. semaine", x = "Consommation d'alcool en semaine", y = "Moyenne des notes", fill = "conso d'alcool") +
    scale_fill_discrete(type = "viridis") + 
    theme(legend.position = "none")
...
### Le sexe et le lieu de vie ont-ils un impact sur les relations familiales ?

Arrivé à l'adolescence, les 
...
Le sexe et le lieu de vie font peu varier les relations familiales. Néanmoins, on peut remarquer qu'en moyenne **les garçons ont de meilleures relations familiales** et que c'est la même pour **les étudiants ruraux**.

### Être plus âge permet-il de mieux s'entendre avec sa famille ?
...
On peut voir que **plus les étudiants viellissent, plus leur relations familiales se détériorent**. De 17 à 20 ans, la moyenne des relations familiales passe de **`r mean_famrel_17`** à **`r mean_famrel_19`**.
...
dfFSPT <- ggplot(df, aes(x = famsize, y = mean_famrel)) +
    ...
    labs(title = "Parents ensembles", x = "Taille de la famille", y = "Relations familiales", fill = "Taille de la famille") +
    scale_fill_discrete(type = "viridis")
...
dfFSPA <- ggplot(df, aes(x = famsize, y = mean_famrel)) +
    geom_bar(stat = "identity", alpha = 0.7, aes(fill = famsize)) +
    labs(title = "Parents séparés", x = "Taille de la famille", y = "Relations familiales", fill = "Taille de la famille") +
    scale_fill_discrete(type = "viridis")
...
Néanmoins en regardant les relations familiales par rapport à la taille de famille, suivant que les parents soient ensembles ou non, **les chiffres deviennent plus interessants** pour les étudiants avec des parents séparés.<br>
Ces derniers ont de **meilleures relations familiales** lorsque la **taille de la famille est égale ou inférieure à 3**.

### L'impact de la taille de la famille sur les relations familiales suivant que les parents soient ensemble ou non
...
dfGPT <- ggplot(df, aes(x = guardian, y = mean_famrel)) + 
  ...
  labs(title = "Parents ensembles", x = "Tuteur légal", y = "Relation familiale", fill = "Tuteur légal") +
  scale_fill_discrete(type = "viridis")
...
dfGPA <- ggplot(df, aes(x = guardian, y = mean_famrel)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = guardian)) +
  labs(title = "Parents séparés", x = "Tuteur légal", y = "Relation familiale", fill = "Tuteur légal") +
  scale_fill_discrete(type = "viridis")
...
dfMJ <- ggplot(df, aes(x = Mjob, y = mean_famrel)) + 
  ...
  labs(title = "Famrel suivant le travail de maman", x = "Travail", y = "Relation familiale", fill = "Travail") +
    scale_fill_discrete(type = "viridis")
...
dfFJ <- ggplot(df, aes(x = Fjob, y = mean_famrel)) + 
  ...
  labs(title = "Famrel suivant le travail de papa", x = "Travail", y = "Relation familiale", fill = "Travail") +
    scale_fill_discrete(type = "viridis")

grid.arrange(dfFJ, dfMJ, ncol=2, nrow = 1)
```
...
ggplot(df, aes(x = Fjob, y = mean_famrel)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Fjob)) +
  labs(title = "Relations familiales suivant le travail de papa (maman dans le service public)", x = "Travail", y = "Relation familiale", fill = "Travail") +
    scale_fill_discrete(type = "viridis")
...
ggplot(df, aes(x = Mjob, y = mean_famrel)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Mjob)) +
  labs(title = "Relations familiales suivant le travail de maman (papa dans le service public)", x = "Travail", y = "Relation familiale", fill = "Travail") +
    scale_fill_discrete(type = "viridis")
```
...
dfME <- ggplot(df, aes(x = Medu, y = mean_famrel)) +
    ...
    labs(title = "Famrel selon l'éducation de maman", x = "Education", y = "Relations familiales", fill = "Education") +
...
dfFE <- ggplot(df, aes(x = Fedu, y = mean_famrel)) +
    ...
    labs(title = "Famrel selon l'éducation de papa", x = "Education", y = "Relations familiales", fill = "Education") +
...
De nos analyses, nous avons pu déceler plusieurs critères qui permettent d'avoir de meilleures relations familiales :
<ul>
...
La santé est quantifiée sur une échelle de 1 à 5, avec 1 signifiant "mauvais" et 5 "excellent".
...
plot1 <- ggplot(data1, aes(x = factor(Pstatus), y = health, fill = Pstatus )) + 
  geom_col() +
  labs( title = "Santé selon la nature de la famille", x = "Type de famille", y =  "État moyen de santé") + 
...
plot1 <- ggplot(data1, aes(x = Pstatus, y = mean_health)) + 
  ...
  labs(title = "Santé selon la nature de la famille", x = "Type de famille", y = "Santé") +
  scale_fill_discrete(type = "viridis") +
  theme(legend.position = "none") +
  scale_x_discrete(labels = c("Classique", "Monoparentale"))
>>>>>>> 706f2d142862fec6a4272da36b21f29c42711d64
...
plot2 <- ggplot(data2, aes(x = guardian, y = mean_health)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = guardian)) +
  labs(title = "Santé selon le tuteur", subtitle = "Familles séparées", x = "Type de famille", y = "Santé") +
  scale_fill_discrete(type = "viridis") +
  theme(legend.position = "none") +
...
En s'interessant de plus près aux familles monoparentales, et que l'on s'interesse aux tuteurs on se rend compte que **les élèves éduqués par une mère seule ont plus de soucis de santé, décrochant de près d'un point**4 des autres cas.<br>
...
plot1 <- ggplot(data1, aes(area = n, fill = factor(Mjob), label = factor(Mjob))) + 
  geom_treemap() +
...
plot2 <- ggplot(data2, aes(x = factor(Mjob) , y = health, fill = factor(Mjob))) + 
  geom_col() + 
  labs(title = "Santé selon le métier de la mère", 
       subtitle = "Familles séparées",
       x = "Travail de la mère",
       y = "État de santé") + 
  theme(legend.position = "none")  
  scale_x_discrete(name = "Secteur d'emploi", labels = c("Au foyer", "Santé","Autre","Services publics", "Enseignement"))

plot3 <- ggplot(data3, aes(area = n, fill = factor(Medu), label = Medu)) + 
  geom_treemap() +
  geom_treemap_text(fontface = "italic", colour = "white", place = "centre") + 
  scale_fill_discrete(name = "Niveau d'études", labels = c("Primaire", "Collège","Lycée","Supérieur"))

plot4 <- ggplot(data4, aes(x = factor(Medu), y = health, fill = factor(Medu))) + 
  geom_col() + 
  labs(title = "Santé selon l'éducation de la mère", 
       subtitle = "Familles séparées",
       x = "Éducation de la mère",
       y = "État de santé") + 
  scale_x_discrete(name = "Niveau d'études", labels = c("Primaire", "Collège","Lycée","Supérieur")) +
  theme(legend.position = "none")
=======
plot2 <- ggplot(data2, aes(x = Mjob, y = mean_health)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Mjob)) +
  labs(title = "Santé selon le métier de la mère", x = "Travail de la mère", y = "Santé") +
  scale_fill_discrete(type = "viridis") +
  theme(legend.position = "none") +
  scale_x_discrete(labels = c("Au foyer", "Santé","Autre","Services publics", "Enseignement"))
...
plot4 <- ggplot(data4, aes(x = Medu, y = mean_health)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Medu)) +
  labs(title = "Santé selon l'éducation de la mère", x = "Études de la mère", y = "Santé") +
  scale_fill_discrete(type = "viridis") +
  theme(legend.position = "none") +
...
ggplot(df, aes(x = famrel, y = mean_health)) +
  ...
  labs(title = "Santé selon la relation familiale", x = "Relations familiales", y = "État de santé") +
  scale_fill_discrete(type = "viridis") +
  theme(legend.position = "none")
...
plot1 <- ggplot(df, aes(x = Mjob, y = mean_health)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Mjob)) +
  labs(title = "Santé suivant le travail de maman", x = "Travail", y = "Santé") +
  scale_fill_discrete(type = "viridis") +
  theme(legend.position = "none") +
  scale_x_discrete(labels = c("Au foyer","Santé", "Autre","Serv.Public","Education"))
...
plot2 <- ggplot(df2, aes(x = Fjob, y = mean_health)) + 
  geom_bar(stat = "identity", alpha = 0.7, aes(fill = Fjob)) +
  labs(title = "Santé suivant le travail de papa", x = "Travail", y = "Santé") +
  scale_fill_discrete(type = "viridis") +
  theme(legend.position = "none") +
  scale_x_discrete(labels = c("Au foyer","Santé", "Autre","Serv.Public","Education"))
...
Effectivement, nous n'observons pas de gros écarts mais nous remarquons tout de même que les parenrs travaillant dans le secteur de la santé ont les enfants présentant les meilleurs résultats sur cet aspect

### Etat de santé en fonction de l'éducation des parents

Ici on peut faire l'hypothèse selon laquelle le fait d'avoir des parents de faible niveau d'éducation aurait des effets néfastes sur la santé des élèves.

```{r health_pedu, message = FALSE}
df <- dfPortugais %>% group_by(Medu) %>% mutate(mean_health = mean(health)) %>% distinct(Medu, mean_health)
...
On se rend compte après étude que l'hypothèse n'est pas complètement vérifiée et on observe des comportements différents selon le sexe du parent. Alors que la santé des enfants de mères ayant le plus bas niveau d'études est la meilleur, celle des pères ayant le plus bas niveau d'études est de façon significative moins bonne que celle des autres.

CONCLURE
<hr>
Plusieurs critères permettent d'avoir une bonne santé. Il s'agit : 
- La qualité de la relation familiale : Mieux elle est, meilleure est la santé.
- Le travail des parents : On constate une meilleure santé lorsque lepère ou la mère travaille dans le secteur de la santé.
- La nature de la famille : En prenant l'évènement contraire on voit que les familles monoparentales où la garde est à la mère rencontre des difficultés et que ca se passe relativement bien dans les autres cas

# Conclusion

Pour conclure sur cette analyse, on peut définir que pour être un élève "*parfait*" sur les plans scolaire, familial et social, les critères à réunir sont :
<ul>
  <li></li>
</ul>

--> critères pour être un élève "parfait" scolaire, familial, médical.

--> point positif : anaylse qui semblait être interessante et qui l'a été. On a réussi à dégager 3 axes de recherche

--> point négatif sur les variables

Enfin, pour conclure plus généralement par rapport au projet

--> Projet qui malgré le travail à distance s'est très bien déroulé

--> 3 personnes, 3 axes de recherches
