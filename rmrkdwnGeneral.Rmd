...
EXPLIQUER LES ECHELLES ALLANT DE 0-4 ET 1-5 !!!!
(quantifier l173 ?)
...
On peut donc voir que **les garcons ont des meilleures notes que les filles**, alors qu'ils travaillent moins en moyenne. On conclue qu'en théorie, **il vaut mieux être un garcon pour réussir**.
...
```{r schoolsup_grade, message = FALSE}
#preparation des données
...
Enfin, on peut voir que les échecs d'un élève **influe directement** sur ses notes. En effet, ce sont **les élèves n'ayant jamais échoués qui s'en sortent le mieux**, cela semble logique.
...
```{r distribution etat santé, message = FALSE}
...
plot
```
L'état de santé moyen est `r statistics$mean` et l'écart type de `r statistics$sd`. La population est majoritairement en bonne santé. Par la suite nous allons présenter l'influence de certains facteurs sur l'état de santé des élèves 

### Exploration de l'effet de la nature de la famille (monoparentale, biparentale) sur la santé des élèves.
...
plot3 <- ggplot(data3, aes(area = n, fill = factor(Medu), label = Medu)) + 
  geom_treemap() +
  geom_treemap_text(fontface = "italic", colour = "white", place = "centre") +
  scale_fill_discrete(name = "Niveau d'études", labels = c("1", "2","3","4"))
...
plot4 <- ggplot(data4, aes(x = factor(Medu) , y = health, fill = Medu)) + 
  ...
  labs(title = "Santé selon l'éducation de la mère", 
       ...
       y = "État de santé") + 
  scale_x_discrete(name = "Niveau d'études", labels = c("1", "2","3","4")) +
  theme(legend.position = "none")
...
plot1 <- ggplot(data1, aes( x = factor(Mjob), y = health, fill = Mjob)) +
  geom_col() + 
  labs(title = "Santé selon le travail de la mère") + 
  scale_x_discrete(name = "Secteur d'emploi", labels = c("Au foyer", "Santé","Autre","Services publics", "Enseignement"))+
  theme(legend.position = "none")
...
plot2 <- ggplot(data2, aes( x = factor(Fjob), y = health, fill = Fjob)) +
  geom_col() + 
  labs(title = "Santé selon le travail du père",
       y = "État de santé") + 
  scale_x_discrete(name = "Secteur d'emploi", labels = c("Au foyer", "Santé","Autre","Services publics", "Enseignement"))+
  theme(legend.position = "none")
...
plot1 <- ggplot(data1, aes( x = Medu, y = health, fill = Medu)) +
  geom_col() + 
  labs(title = "Santé selon le niveau d'études de la mère",
       x = "Niveau d'études",
       y = "État de santé") + 
  theme(legend.position = "none")
...
plot2 <- ggplot(data2, aes( x = Fedu, y = health, fill = Fedu)) +
  geom_col() + 
  labs(title = "Santé selon le niveau d'études du père",
       x = "Niveau d'études",
       y = "État de santé") + 
  theme(legend.position = "none")
